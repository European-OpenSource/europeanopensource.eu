...
convert_json_to_md() {
  ...
  local norm_name=$(echo "${name}" | tr '[:upper:]' '[:lower:]' | sed 's/[^a-z0-9]/-/g' | sed 's/-\{2,\}/-/g' | sed 's/^-//' | sed 's/-$//')
  local base_file=$(basename "${json_file}" .json)
  local md_file="${ROOT_PATH}/lib/database/projects/${norm_name}-${base_file:0:5}.md"
